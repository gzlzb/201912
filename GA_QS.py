...
Correct = pd.read_csv('Correct_1.csv', sep=',')
Predict = pd.read_csv('Predict_1.csv', sep=',')
...
def q2ven(cor):
    ...
    PreAll = pd.read_csv('Predict_1.csv', sep=',')
    cols = set(cor.columns)
    pre = pd.DataFrame(PreAll[cols])
    
    val = pd.DataFrame(columns=['ture', 'pred'])
    import warnings
    warnings.filterwarnings("ignore")
    X_pre = pre.drop(["LogBIO"], axis=1)
